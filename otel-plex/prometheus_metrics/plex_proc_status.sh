...
# Output metric file
METRIC_FILE="<PATH_TO_YOUR_METRICS>/plex_proc.prom"

# Input file (powermetrics output)
POWERMETRICS_FILE="<PATH_TO_YOUR_METRICS>/powermetrics.out"


# Extract Plex Media Server process metric (e.g. CPU/user column)
PROM_METRIC=$(grep "Plex Media Server" "$POWERMETRICS_FILE" | head -n1 | awk '{print $4}')
...
echo "plex_proc_user $PROM_METRIC" > "$METRIC_FILE"
