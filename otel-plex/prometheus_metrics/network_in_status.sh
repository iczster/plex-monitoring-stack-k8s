...
# Output metric file
METRIC_FILE="<PATH_TO_YOUR_METRICS>/network_in.prom"

# Input file (powermetrics output)
POWERMETRICS_FILE="<PATH_TO_YOUR_METRICS>/powermetrics.out"


# Extract inbound network throughput (bytes/sec)
PROM_METRIC=$(grep "in:" "$POWERMETRICS_FILE" | head -n1 | awk -F, '{print $2}' | awk '{print $1}')
...
echo "network_in_total_bytes_per_second $PROM_METRIC" > "$METRIC_FILE"
