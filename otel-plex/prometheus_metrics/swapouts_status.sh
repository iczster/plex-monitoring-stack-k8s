...
# Output metric file
METRIC_FILE="<PATH_TO_YOUR_METRICS>/swapouts.prom"


# Extract total swapouts from vm_stat
PROM_METRIC=$(/usr/bin/vm_stat | grep "Swapouts:" | awk '{sub(/\.$/, ""); print $2}')
...
echo "total_swapouts $PROM_METRIC" > "$METRIC_FILE"
