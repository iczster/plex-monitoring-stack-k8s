...
# Output metric file
METRIC_FILE="<PATH_TO_YOUR_METRICS>/pages_free.prom"


# Extract free pages from vm_stat
PROM_METRIC=$(/usr/bin/vm_stat | grep "Pages free:" | awk '{sub(/\.$/, ""); print $3}')
...
echo "total_pages_free $PROM_METRIC" > "$METRIC_FILE"
