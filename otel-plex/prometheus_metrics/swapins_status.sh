...
# Output metric file
METRIC_FILE="<PATH_TO_YOUR_METRICS>/swapins.prom"


# Extract total swapins from vm_stat
PROM_METRIC=$(/usr/bin/vm_stat | grep "Swapins:" | awk '{sub(/\.$/, ""); print $2}')
...
echo "total_swapins $PROM_METRIC" > "$METRIC_FILE"
