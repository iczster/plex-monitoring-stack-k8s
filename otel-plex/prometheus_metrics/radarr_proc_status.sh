...
# Output metric file
METRIC_FILE="<PATH_TO_YOUR_METRICS>/radarr_proc.prom"

# Input file (powermetrics output)
POWERMETRICS_FILE="<PATH_TO_YOUR_METRICS>/powermetrics.out"


# Extract Radarr process metric (e.g. CPU/user column)
PROM_METRIC=$(grep "Radarr" "$POWERMETRICS_FILE" | head -n1 | awk '{print $4}')
...
echo "radarr_proc_user $PROM_METRIC" > "$METRIC_FILE"
