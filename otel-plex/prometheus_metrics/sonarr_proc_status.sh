...
# Output metric file
METRIC_FILE="<PATH_TO_YOUR_METRICS>/sonarr_proc.prom"

# Input file (powermetrics output)
POWERMETRICS_FILE="<PATH_TO_YOUR_METRICS>/powermetrics.out"


# Extract Sonarr process metric (e.g. CPU/user column)
PROM_METRIC=$(grep "Sonarr" "$POWERMETRICS_FILE" | head -n1 | awk '{print $4}')
...
echo "sonarr_proc_user $PROM_METRIC" > "$METRIC_FILE"
